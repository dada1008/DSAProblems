 ...
 *
 * For a given positive number N in base-10, return the complement of
 * its binary representation as a base-10 integer.
 *
 * Example 1:
 *
 * Input: 8
 ...
 *
 * It will return 0 if we take XOR of two same bits i.e. 0^0 = 1^1 = 0. In other words,
 * XOR of two same numbers is 0.
 *
 * It returns the same number if we XOR with 0.
 *
 * From the above-mentioned first property,
 * we can conclude that XOR of a number with its complement will result in a number
 * that has all of its bits set to 1. For example, the binary complement of “101” is “010”;
 * and if we take XOR of these two numbers, we will get a number with all bits set to 1,
 * i.e., 101 ^ 010 = 111
 *
 ...
 * One way to calculate all_bits_set will be to first count the bits required to store the given number.
 * We can then use the fact that for a number which is a complete power of ‘2’ i.e.,
 * it can be written as pow(2, n), if we subtract ‘1’ from such a number,
 * we get a number which has ‘n’ least significant bits set to ‘1’.
 * For example, ‘4’ which is a complete power of ‘2’, and ‘3’ (which is one less than 4)
