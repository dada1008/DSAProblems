 ...
 * their frequencies so that the root of the heap is always the most frequently occurring number.
 * There are two issues that need to be resolved though:
 *
 * How can we keep track of the frequencies of numbers in the heap? When we are pushing
 * a new number to the Max Heap, we don’t know how many times the number has already appeared
 * in the Max Heap. To resolve this, we will maintain a HashMap to store the current frequency
 * of each number. Thus whenever we push a new number in the heap, we will increment its frequency
 * in the HashMap and when we pop, we will decrement its frequency.
